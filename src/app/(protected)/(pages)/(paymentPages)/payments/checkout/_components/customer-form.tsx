...
import { CountrySelect } from "@/components/global/country-selector/countrySelector";
import { useAuth, useClerk } from "@clerk/nextjs";

import axios from "axios";
import { ProductPartProps } from "@/lib/types";
...
const CustomerPaymentForm =  ({ProductDetails}: ProductPartProps) => {
  ...
  const createPaymentLink = async (formData: z.infer<typeof formSchema>) => {
    try {
      const response = await buySubscription(formData,ProductDetails.product_id)

      if (response.status !== 200 || !response.paymentLink ) {
                  ...
                  <FormControl>
                    <PhoneInput
                      ...
                      countrySelectorStyleProps={{
                        buttonClassName:
                          "border border-input px-2 hover:bg-accent",
                      }}
                      inputProps={{
                        className:
                          "flex h-9 w-full rounded-r-md border border-input px-3 py-2 text-sm",
                      }}
                    />
                  </FormControl>
                </FormItem>
